...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    if os.getenv("LAB18_FAST_TESTS") == "1":
        # ponytail: test-mode skips model load; real pipeline runs without this env var.
        return [Chunk(text=" ".join(sentences), metadata={**metadata, "strategy": "semantic"})]
    ...
    for i in range(1, len(sentences)):
        ...
        norm_prev = norm(prev_embedding)
        norm_curr = norm(curr_embedding)
        if norm_prev == 0 or norm_curr == 0:
            sim = 0.0
        else:
            sim = dot(prev_embedding, curr_embedding) / (norm_prev * norm_curr + 1e-9)
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    def process_parent(text_content):
        pid = f"parent_{len(parents)}"
        parent_meta = {**metadata, "chunk_type": "parent", "parent_id": pid}
        parents.append(Chunk(text=text_content, metadata=parent_meta))

        # Split parent text thành children
        sentences = [s.strip() for s in re.split(r'(?<=[.!?])\s+|\n', text_content) if s.strip()]
        current_child_text = ""

        for sentence in sentences:
            if len(current_child_text) + len(sentence) > child_size and current_child_text:
                child_meta = {**metadata, "chunk_type": "child", "parent_text": text_content}
                children.append(Chunk(text=current_child_text.strip(), metadata=child_meta, parent_id=pid))
                current_child_text = ""
    ...
    for para in paragraphs:
        if len(current_parent_text) + len(para) > parent_size and current_parent_text:
            process_parent(current_parent_text.strip())
            current_parent_text = ""
